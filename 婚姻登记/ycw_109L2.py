...
def yzm(cancel):
    ...
    if cancel == None:  # 进行登录验证码破解
        ...
        print('秒杀来验证码')
        if os.name == 'posix':
            with open('./证件信息3.txt', 'r', encoding='utf‐8') as a_file:
                cookie = a_file.readline().rstrip()
        else:
            with open(r"./证件信息3.txt", 'r', encoding='utf‐8') as a_file:
                cookie = a_file.readline().rstrip()
        headers = {
    ...
    if cancel == None:
        ...
        if os.name == 'posix':
            ...
            with open('./证件信息3.txt', mode='w', encoding='utf‐8') as a_file:
                a_file.write(f'JSESSIONID={values};openstack_cookie_insert={values2}')
                # 写字
        else:
            with open(r'./证件信息3.txt', mode='w', encoding='utf‐8') as a_file:
                a_file.write(f'JSESSIONID={values};openstack_cookie_insert={values2}')
            # 写字
    return zhi
