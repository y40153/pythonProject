...
def yzm(cancel):
    ...
    if cancel == None:  # 进行登录验证码破解
        ...
        print('秒杀来验证码')
        if os.name == 'posix':
            with open('./证件信息1.txt', 'r', encoding='utf‐8') as a_file:
                cookie = a_file.readline().rstrip()
        else:
            with open(r"C:\Users\Administrator\Desktop\证件信息1.txt", 'r', encoding='utf‐8') as a_file:
                cookie = a_file.readline().rstrip()
        headers = {
    ...
    if cancel == None:
        ...
        if os.name == 'posix':
            ...
            with open('./证件信息1.txt', mode='w', encoding='utf‐8') as a_file:
                a_file.write(f'JSESSIONID={values};openstack_cookie_insert={values2}')
                # 写字
        else:
            with open(r'C:\Users\Administrator\Desktop\证件信息1.txt', mode='w', encoding='utf‐8') as a_file:
                a_file.write(f'JSESSIONID={values};openstack_cookie_insert={values2}')
            # 写字
    return zhi
...
def query(shijian, bianhao, weizhi):
    ...
    if os.name == 'posix':
        ...
        with open(r"C:\Users\Administrator\Desktop\证件信息1.txt", 'r', encoding='utf‐8') as a_file:
            cookie = a_file.readline().rstrip()
    payload = f'djjgbm={bianhao}0A0000&ywlx=J&rqDate={shijian}&ydbllx=01'
...
def chaxun():
    ...
    zi = 1
    while True:
        key = query(date, '440112', '黄埔区')
            #query(date, '440103', '荔湾区')+ query(date, '440112', '黄埔区')+ query(date, '440104', '越秀区')+ query(date,'440106', '天河区')
        # key = query(date, '440396', '大鹏新区') + query(date, '440112', '盐田区') + query(date,'440106', '龙岗区') key =
        ...
        print(f'孙莉媛{sj},第{zi}次轮询：有{key}个区有号')
        zi += 1
        if key > 0:
            global name
            print('发邮件哦', name)
...
def run(yyrq, shij, bianhao, diz):
    seckill(f'{yyrq}', f'{shij}', f'{bianhao}', f'{diz}',
            '邓宝军', '432802196201182994', '13128278777',
            '孙莉媛', '445201198810120064', '13642658088',
            32)
    return
