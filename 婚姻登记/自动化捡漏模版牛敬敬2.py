...
def chaxun():
    ...
    zi = 1
    while True:
        key = query(date, '440303', '罗湖区')+ query(date,'440311', '光明区')+ query(date, '440310', '坪山')
        # key = query(date, '440305', '南山区')+ query(date, '440396', '大鹏新区') + query(date, '440308', '盐田区')+ query(date,'440307', '龙岗区')
        ...
        print(f'牛敬敬{sj},第{zi}次轮询：有{key}个区有号')
        zi += 1
        if key > 0:
            print('发邮件哦', name)
            sendmail(name)
...
def run(yyrq, shij, bianhao, diz):
    ...
    #         32)
    seckill(f'{yyrq}', f'{shij}', f'{bianhao}', f'{diz}',
            '曹军军', '342201199411118539', '13519929593',
            '牛敬敬', '342201199508054420', '13823330847',
            32)
    return
