...
def seckill(date, time, bianhao, dizhi, manname, manhao, phone, wumanname, wumanhao, phone2, colour):
    ...
    payload = json.dumps({
        ...
        "hydjEnty": {
            ...
            "area_citynan": "441700000000",
            "area_citynv": "441700000000",
    ...
    if cancel == None:  # 进行登录验证码破解
        ...
        if os.name == 'posix':
            ...
            with open(r"./证件信息.txt", 'r', encoding='utf‐8') as a_file:
                cookie = a_file.readline().rstrip()
        headers = {
            'Referer': 'https://www.gdhy.gov.cn/yyjh.do?do=preYyxxOper&yyrq=2023-08-30&djjg=4406060A1000&yysj=9:00-10:00&ydbllx=01',
            'Cookie': f'{cookie}'
        }
...
def query(shijian, bianhao, weizhi):
    ...
    headers = {
        ...
        'Referer': 'https://www.gdhy.gov.cn/common.do?do=getWdrqxx&yyrq=2023-08-30&blcs=4417020A1000&ywlx=LS',
        'Accept-Encoding': 'gzip, deflate, br',
...
def chaxun():
    ...
    zi = 1
    while True:
        key =query('2023-01-03', '441702', '阳江市江城区民政局婚姻登记处')+query('2023-01-04', '441702', '阳江市江城区民政局婚姻登记处')+query('2023-01-05', '441702', '阳江市江城区民政局婚姻登记处')+query('2023-01-06', '441702', '阳江市江城区民政局婚姻登记处')
        ...
        print(f'刘观想{sj},第{zi}次轮询：有{key}个区有号')
        zi += 1
        if key > 0:
            global name
            print('发邮件哦', name)
...
def run(yyrq, shij, bianhao, diz):
    ...
    seckill(f'{yyrq}', f'{shij}', f'{bianhao}', f'{diz}',
            '刘观想', '441702198508051416', '18420577263',
            '甘文琦', '441702198610090729', '18934125066',
            32)
    return
